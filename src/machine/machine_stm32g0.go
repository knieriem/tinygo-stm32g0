...
func (spi SPI) getBaudRate(config SPIConfig) uint32 {
	...
	// speeds due to the CPU freqency, pick a range up to that frequency for
	// clients to use more human-understandable numbers, e.g. nearest 100KHz

	// These are based on APB2 clock frquency (16MHz on the discovery board)
	// TODO: also include the MCU/APB clock setting in the equation
	switch {
	case localFrequency < 250_000:
		conf = stm32.SPI_CR1_BR_Div64
	case localFrequency < 500_000:
		conf = stm32.SPI_CR1_BR_Div32
	case localFrequency < 1_000_000:
		conf = stm32.SPI_CR1_BR_Div16
	case localFrequency < 2_000_000:
		conf = stm32.SPI_CR1_BR_Div8
	case localFrequency < 4_000_000:
		conf = stm32.SPI_CR1_BR_Div4
	case localFrequency < 32_000_000:
