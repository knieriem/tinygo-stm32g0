...
func initCLK() {
	...
	// Set flash wait states (min 1 latency units) based on clock
	if (stm32.FLASH.ACR.Get() & 0x7) < 1 {
		stm32.FLASH.ACR.ReplaceBits(1, 0x7, 0)
	}
	...
	// Set flash wait states (max 1 latency units) based on clock
	if (stm32.FLASH.ACR.Get() & 0x7) > 1 {
		stm32.FLASH.ACR.ReplaceBits(1, 0x7, 0)
	}

	// Set APB clock (0 = DIV1)
